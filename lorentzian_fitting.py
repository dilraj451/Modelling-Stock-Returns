...
y = p_opt[0]*(((p_opt[2])**2)/((x-p_opt[1])**2+((p_opt[2])**2)))
plt.plot(x,y,color='red')
plt.show()

print('Amplitude: {} +\- {}'.format(p_opt[0], np.sqrt(p_cov[0,0])))
print('Median: {} +\- {}'.format(p_opt[1], np.sqrt(p_cov[1,1]))) 
print('0.5*FWHM: {} +\- {}'.format(p_opt[2], np.sqrt(p_cov[2,2]))) 
...
ks_test= kstest(returns3,'cauchy',args=(p_opt[1],p_opt[2]))
